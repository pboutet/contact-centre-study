...
class RegLSTM(nn.Module):
    ...
    def forward(self, x, lengths=None):
        ...
        if lengths is not None:
            x = torch.nn.utils.rnn.pack_padded_sequence(x, lengths.cpu(), batch_first=True)# bug fix https://github.com/pytorch/pytorch/issues/43227
        ...
        if len(list(self.parameters())) != len(self.avg_param) and self.beta_ema > 0:
            self.steps_ema = 0.
            self.avg_param = deepcopy(list(p.data for p in self.parameters()))
            if torch.cuda.is_available():
                self.avg_param = [a.cuda() for a in self.avg_param]
