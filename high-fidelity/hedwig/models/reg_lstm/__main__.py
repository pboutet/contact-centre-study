...
from datasets.litcovid import LitCovid
from datasets.yelp2014 import Yelp2014
from datasets.lulu import lulu#added lulu entry
...
def evaluate_dataset(split_name, dataset_cls, model, embedding, loader, batch_size, device, is_multilabel, args=None):
    saved_model_evaluator = EvaluatorFactory.get_evaluator(dataset_cls, model, embedding, loader, batch_size, device, args)
    ...
    #print(metric_names)
    #print(scores)
    #added evaluation report alongside other metrics
    print(metric_names[0:5])
    print(scores[0:5])
    print(metric_names[5])
    print(scores[5])
    ...
    if args.dataset not in dataset_map:
        ...
        if args.extra_test_set is not None:
            train_iter, dev_iter, test_iter = dataset_class.iters(args.data_dir,
                                                              ...
                                                              device=args.gpu,
                                                              unk_init=UnknownWordVecCache.unk,
                                                              extra_test_set=args.extra_test_set)
