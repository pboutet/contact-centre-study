...
from datasets.bert_processors.litcovid_processor import LitCovidProcessor
...
from datasets.bert_processors.lulu_processor import luluProcessor
...
def evaluate_split(model, processor, tokenizer, args, split='dev'):
    ...
    accuracy, precision, recall, f1, avg_loss, classification_report = evaluator.get_scores(silent=True)[0]
    print('\n' + LOG_HEADER)
...
if __name__ == '__main__':
    ...
    if not args.trained_model:
        train_examples = processor.get_train_examples(args.data_dir, args.training_file)
        num_train_optimization_steps = int(
            len(train_examples) / args.batch_size / args.gradient_accumulation_steps) * args.epochs

    model = SequenceClassification(args.num_labels, args)

    tokenizer = RobertaTokenizer.from_pretrained('roberta-base')
    tokenizer.model_max_length = model.longformer.config.max_position_embeddings
